...
class OutputAnalyzer {
    private final HeartbeatActivity activity;
    ...
    OutputAnalyzer(HeartbeatActivity activity, TextureView graphTextureView, Handler mainHandler) {
    ...
    void measurePulse(TextureView textureView, CameraService cameraService) {
        ...
        timer = new CountDownTimer(measurementLength, measurementInterval) {
            ...
            public void onFinish() {
                ...
                sendMessage(HeartbeatActivity.MESSAGE_UPDATE_FINAL, returnValueSb.toString());

                cameraService.stop();
            }
        };

        activity.setViewState(HeartbeatActivity.VIEW_STATE.MEASUREMENT);
        timer.start();
    }
